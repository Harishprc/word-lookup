"""Cache sync with the Android app via one private GitHub Gist.
...
with merge-on-pull, not a database and not realtime sync. Same design and
same wire format as the Android side's `data/sync/` package; this module
is the Python mirror of `GistSyncClient.kt` + `SyncMerger.kt`.
...
`sync_now` becomes a no-op; nothing else about the app changes.
...
Field names are camelCase (matching Kotlin's `SyncEntry`, whose
kotlinx.serialization defaults encode property names verbatim) even
though the rest of this Python codebase is snake_case — this module is
the one place that speaks the shared wire format, so the mismatch is
contained here rather than leaking into store.py. Timestamps are epoch
**milliseconds** on the wire (Android's native `System.currentTimeMillis`
unit); this module converts to/from the desktop store's epoch-**seconds**
floats at the boundary (`_to_wire` / `_from_wire`).
"""
...
def _winner(a: dict, b: dict) -> dict:
    ...
    deterministic tie-break — identical rule to Kotlin's `SyncMerger`."""
